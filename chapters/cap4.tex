...
se pode considerar que essa funcionalidade é útil (ela é constantemente utilizada pelos processos de ETL);
...
\textbf{Validade interna:} como mencionado na parte "Seleção dos indivíduos" para o estudo propõe-se a utilizar ferramentas presentes na literatura, que são validadas pelo meio acadêmico. Assim, assume-se que elas são representativas para a população de ferramentas de ETL.
...
\begin{table}[h!]
	...
	%	\multicolumn{6}{|c|}{Utilidade}\\
		...
		&&&&& - É uma funcionalidade trivial, pois é possível comparar os dados da fonte e destino mesmo sem \textit{debugging}.\\
